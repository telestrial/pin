...
Pin uses the Sia Storage SDK **at its native source**: the Rust [`sia_storage`](https://github.com/SiaFoundation/sia-sdk-rs) crate, wrapped in [`crates/pin-sia`](crates/pin-sia/src/lib.rs) and compiled two ways — to WebAssembly for the browser, and linked natively into the desktop shell. The npm `@siafoundation/sia-storage` package is itself built from that same crate, so this is the same SDK taken one layer down. Every call below has **one implementation** serving both targets.

| SDK call | Where it's used |
| --- | --- |
| `Sdk` instance per user | Every authenticated session. The `Builder` typestate also *is* onboarding — `request_connection` → `wait_for_approval` → `register` are the Connect / Approve / Recovery-phrase screens, and `connected(app_key)` is the restore path on every later boot |
| `sdk.upload(Object::default(), stream)` | Post body bytes, every attachment file on a post, channel cover art, profile avatar / cover image bytes. The composer never blocks on upload — bytes get pushed to Sia by the background runner after Publish, then the manifest is written with the resolved URLs. |
| `sdk.pin_object(obj)` | Durability for items you publish (body + each attachment); mirroring a friend's whole post (body + every attachment) into your storage when you pin it; mirroring just **one attachment file** into your Library on its own (the post vs the file are separate custody relationships — *a post is something you authored, a file is bytes that exist*); and **channel-level pin** — a snapshot that fans the same mirror out over every item in a channel, with per-item progress shown in the right sidebar and the header pin filling as it goes. Re-pinning a channel catches up newly-published items |
| `sdk.share_object(obj, valid_until)` | Per-item distribution URL with the per-object encryption key in the URL fragment; year-9999 expiries verified safe |
| `sdk.shared_object(url)` | Resolves a shared URL into an object handle. Used before downloading (subscriber reads) and before mirroring (pinning a friend's item — `shared_object` then `pin_object` adds the bytes to your indexer scope) |
| `sdk.download(obj)` (read as a stream) | Subscriber reads (cached in IndexedDB after first fetch — see Architecture) |
| `sdk.delete_object(id)` | Retraction at three granularities: a single **attachment** (drop one file from your post), a whole **item** (typed-`DELETE` on your own post), and a whole **channel** (**Unpin channel** walks every item, then deletes the manifest objects; the DHT pointer expires by TTL). Retraction is **eager and reference-safe** — a decision you made frees the bytes immediately, but skips any object still referenced by another of your posts or a pin (content-addressing means the same file can be shared). A subscriber's unpin is reference-aware the same way (a file held by both a whole-post pin and a standalone file pin survives until the last holder lets go). Subscribers who mirrored items keep their independent copies |
| `sdk.upload_packed()` + `packed.add(stream)` + `packed.finalize()` | Bin-packed multi-object uploads. Composer tasks (post body + attachments) and drag-drop intake into My Storage share a single ~40 MiB slab rather than one slab per object. Same call powers the background repack runner that consolidates sub-full slabs after the fact |
...
| `sdk.object_events(cursor, limit)` | Paginated walk of every pinned object in scope. Used to total raw content bytes for the storage meter, by the repack runner to inventory slabs, and by full-reset to enumerate every object to delete |
| `sdk.prune_slabs()` | Releases unused slab capacity after `delete_object`. The indexer doesn't auto-drop empty slabs, so without this `pinnedData` stays inflated even after deletes succeed. Called at the end of every repack batch — load-bearing for the bar to actually fall |
...
| `AppKey::public_key()` | Recorded inside the encrypted channel manifest as the technical author identity |
...
**Identity layer.** A person's identity is a **`did:dht`** — an ed25519 key derived from their Sia recovery phrase (HKDF), the same key whether they're on the web app or the desktop shell. No account, no company, no atproto handle in the path. Everything below hangs off it:
...
- **Resolution ladder + stale-while-revalidate** (`core/feed.ts`, `stores/feed.ts`, `lib/channelLocator.ts`, `lib/channelRevalidate.ts`) — reading a channel walks a preference ladder and takes the best rung available right now, caching what it resolves so the next reader lands higher. For a **subscribed** channel that's: serve the locally-cached manifest if there is one (instant, no network) → else resolve its DHT locator → Sia → decrypt, and cache the result. Channels you **own** always resolve fresh, since their newest state is the one you just published. Since a cached read can't itself notice that an author published, a background pass re-resolves each subscription on a cadence and **fills the feed in** when the content actually moved — no Refresh required. Because Mainline is eventually-consistent, a channel that momentarily fails to re-resolve keeps its last-known manifest instead of dropping out of the feed; a subtle "Refreshing…" line shows a background re-resolve over already-visible content. The cache is shared between your own devices, so an always-on machine keeps it warm for a phone that's only awake for a minute.
- **Live-sync from the author** (`lib/channelDoc.ts`, `crates/pin-core`, `src-tauri/src/docstore.rs`) — the ladder's top rung, and the only one that doesn't wait for a poll. An author also serves each channel as its own [iroh-docs](https://www.iroh.computer) replica — the same manifest ciphertext, held in a doc — and publishes a read capability for it to the DHT under a second `K`-derived key. A subscriber holding `K` derives that key, resolves the capability, imports the replica, and from then on the author's writes are **pushed** to them, landing on the same fill-in path the background pass uses. Two properties make it safe to lean on: the capability is **read-only**, because the doc's namespace secret (which *is* the write capability in iroh-docs) is derived from the author's own key and never shared — so a subscriber can read the channel but can never write to it; and the whole rung is **additive**, so a channel whose author has never run an instance, or is simply offline, keeps resolving through the rungs below with nothing to configure. One replica per channel rather than one per identity, for the same reason each channel gets its own key: a read capability covers a whole namespace, so sharing one identity-wide doc would expose every *other* channel's existence to anyone you let read one of them.
...
**One data layer, in Rust, compiled twice.** Everything that requires the user's keys to do — storing bytes, sealing manifests, signing DHT records, resolving a channel, keeping subscriptions current — lives in a set of Rust crates compiled both to WebAssembly (the browser) and natively (the desktop shell). Not two implementations behind one interface: one implementation, two targets. What differs between them is only what is *physically* different — a browser can't send UDP, so it reaches the DHT through a relay while the desktop talks to Mainline directly; a browser tab can't stay awake, so the desktop's copy of the same loop simply runs longer. Nothing is a lesser tier.

```
crates/
  pin-sia       # the Sia SDK: upload, pin, share, download, delete, prune, the account walk
  pin-crypto    # content hashing (CIDv1), the AES-GCM envelope, channel-ID derivation
  pin-derive    # every secret the one root AppKey implies (HKDF, domain-separated)
  pin-pkarr     # signed DHT records — relays on wasm, Mainline natively
  pin-manifest  # the channel manifest type and its transforms (publish, edit, retract)
  pin-channel   # a channel's round trip: seal → store → publish pointer, and back
  pin-curator   # the loops that keep things current without anyone watching
  pin-core      # the iroh-docs replica + the wasm bindings the browser calls
  pin-rpc       # the /hey inbox protocol, shared by both engines

src/
  core/        # types, feed collation, pin, profile — plus thin bindings onto the crates
  components/  # web UI (React)
  stores/      # Zustand — auth, feed, pin, action queue, compose (armed-link state), toast
  lib/         # item cache, markdown, hooks, and the per-platform transports
src-tauri/     # the desktop shell + the always-on Curator
```

The UI's job is attention — rendering, navigation, the half-typed draft. The Curator's job is agency: it holds the credentials, so it *is* the user acting. A future native client is therefore a UI layer over the same crates, not a port of the data layer.
...
| Integration | `bun run test:int` | ~3s | In-memory fakes — `FakeSiaClient` plus a fake channel-locator layer on the shared `FakeWorld`, so a channel's publish/resolve round trip runs without the network (and seals with the *real* crypto, so a cached blob is one the app can actually open). Drives the real React components ([HomeFeed](src/components/HomeFeed.tsx), [PinButton](src/components/pin/PinButton.tsx), etc.) through the upload / pin / drift / retract flows. Two simulated accounts in one Node process; cross-account custody is testable in milliseconds. |
...
| Sync | `bun run test:sync` | ~1m | Real iroh relays and the real DHT, **no credentials** — so it runs anywhere. Drives the app's own sync verbs in a real browser: two tabs of one identity converging bidirectionally from a single ticket import, the `#synctest` panel end to end, rendezvous auto-discovery (publish a ticket to a signed DHT record, resolve it from another instance, sync with no manual copy), and the two channel-doc specs — one at the seam (a browser-minted read capability imported by another browser instance, and the read replica refusing a write **as app code sees it**), one end to end (an author publishes, and a second instance finds the channel **from its key alone** and has the manifest pushed into its feed). Its own Playwright config on a dedicated port, so it never touches a running dev server. |
...
- **Native mobile.** The data layer is Rust and already compiles for two targets, so a phone is a third one rather than a port — the path is the same Tauri shell that runs on desktop, which targets iOS and Android too. This also routes around a real limit: every iOS browser is WebKit underneath, and WebKit currently ships neither WebTransport nor readable byte streams, so mobile *web* can't reach Sia hosts at all today. Native isn't a nicety there, it's the only door — and it needs no second implementation to open.
...
There's also a desktop shell in the repo (`src-tauri/`) — the same React app in a native window, with the Curator running behind it as a real always-on process rather than a tab. `bun run dev:desktop` builds and launches it. It isn't packaged or distributed yet; the web app is the thing to look at.

A note on browser support: we built against Chrome throughout the original 3-day window and never widened. Chrome was the target, it was green from day one, and we kept iterating. Reaching Sia hosts needs WebTransport, which rules out Safari and every iOS browser (all WebKit underneath) — see the mobile roadmap entry. The app may run in other Chromium browsers, but Chrome is the only environment we exercised.
