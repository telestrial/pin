...
- **Post editing.** Channel metadata (name, description, cover image) is fully editable from the channel header. Posts aren't editable yet — to change one, retract and republish. The architecture already supports an in-place edit pattern (upload new bytes, swap the manifest pointer on success, roll back on failure, preserve `publishedAt` so edits don't republish into the top of the feed), and that machinery will return as a unified post editor once the UI for it is settled. **Pinned copies on subscribers' devices are *snapshots*** — an author's edit changes the manifest's pointer, but readers' mirrored bytes stay as they were when they pinned. That's true to "K is custody, not authorship": the reader's pin is fixed-in-time stewardship; the author's edit doesn't reach into the reader's library.
