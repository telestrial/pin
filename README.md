...
- **Resolution ladder + stale-while-revalidate** (`core/feed.ts`, `stores/feed.ts`, `lib/channelLocator.ts`, `lib/channelRevalidate.ts`) — reading a channel walks a preference ladder and takes the best rung available right now, caching what it resolves so the next reader lands higher. For a **subscribed** channel that's: serve the locally-cached manifest if there is one (instant, no network) → else resolve its DHT locator → Sia → decrypt, and cache the result. Channels you **own** always resolve fresh, since their newest state is the one you just published. Since a cached read can't itself notice that an author published, a background pass re-resolves each subscription on a cadence and **fills the feed in** when the content actually moved — no Refresh required. Because Mainline is eventually-consistent, a channel that momentarily fails to re-resolve keeps its last-known manifest instead of dropping out of the feed; a subtle "Refreshing…" line shows a background re-resolve over already-visible content. The cache is shared between your own devices, so an always-on machine keeps it warm for a phone that's only awake for a minute. The ladder's top rung — live-sync straight from an author's node, pushing updates instead of polling for them — is future work; it lands on the same fill-in path the background pass uses today.
