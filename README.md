...
| `sdk.objectEvents(cursor, limit)` | Paginated walk of every pinned object in scope. Used to total raw content bytes for the storage meter, by the repack runner to inventory slabs, and (during the one-shot Sia→PDS migration) by the legacy settings loader to find the old settings object |
...
- **Handle directory.** Clicking any `@handle` anywhere opens that handle's directory: a profile header up top (avatar + cover + displayName + bio with @handle fallback, plus a Following count), then the channels they publicly claim rendered as cover-forward hero cards (a public self-follow whose `authorDID` matches their own is the channel-as-voice claim of authorship), and a **Following** section listing everyone else they follow — both derived from the same public-follow walk. On your own directory a Create-a-channel affordance sits above the cards. Reached via in-app navigation; the directory is *how Pin renders any handle a user encounters in-app*, not a paste-this-URL surface.
...
- **Follow the channel vs follow the person.** Channel-follow (above) tracks one voice's output. **Handle-follow** — a `dev.sia.pin.handlefollow` record whose subject is a person's DID — tracks the whole identity: clicking Follow on someone's handle directory auto-Watches every public channel they currently claim and picks up new ones they claim later (reconciled on load). Unfollowing sweeps all their feeds back out. A channel you explicitly unsubscribe stays gone even while you still follow the person — a tombstone set (synced in your settings so it holds across devices) keeps the reconcile from re-adding it.

Pin's OAuth scope is correspondingly narrow — `repo:dev.sia.pin.channel repo:dev.sia.pin.profile repo:dev.sia.pin.subscription repo:dev.sia.pin.handlefollow repo:dev.sia.pin.settings`. The auth screen at bsky.social grants access only to the lexicons Pin actually writes to. No `app.bsky.*` surface.
...
- **Settings durability** (`core/settingsRecord.ts`, `lib/hooks/useSettingsSync.ts`, `lib/useGhostReconciliation.ts`) — your channel list and subscriptions (and the channel keys inside them), plus identity-wide UI preferences like the visual theme, live in a single **mutable encrypted atproto record** at `at://<did>/dev.sia.pin.settings/self`. Mutable state belongs on the PDS, same as channel manifests; overwriting one record in place is what dissolves the whole class of problems content-addressed Sia settings had (every save minting a new object, several coexisting when a delete failed, the load flip-flopping between stale copies). The record body is AES-GCM-256 ciphertext under a key **derived from the Sia AppKey via HKDF** (never the atproto identity, never shared) — so it survives a localStorage wipe and recovers from the Sia recovery phrase alone. The plaintext is **padded to a fixed 128 KiB** before encryption, so the public record's size leaks nothing about how many channels or subscriptions it holds. Writes are **awaited (no debounce) with compare-and-swap** on the record CID — a channel-list change is durable on the PDS before the operation reports done, killing the reload-within-the-debounce-window resurrection bug. Just-reading users (Sia AppKey, no atproto session) keep settings in localStorage only. On load, **ghost channels** — left when a retract deleted the channel record but its settings write then failed, orphaning the list entry — are detected by a *definitive* record-not-found and pruned (never on a transient error, so a network blip can't delete a real channel).
...
| E2E | `bun run test:e2e` | minutes | Real Sia hosts + real bsky.social, driven by Playwright Chrome against the built `dist/` (preview server). Three happy-path tests, serialized (one worker — they share the test accounts): alice publishes / bob subscribes-and-pins-the-channel; an interrupted publish resuming from its IndexedDB checkpoint on reload; and granular pinning (bob pins one attachment file into his Library independent of any whole-post pin, cross-account). Plus a guarded maintenance task that drains accumulated test channels. |
