...
| E2E | `bun run test:e2e` | minutes | Real Sia hosts + real bsky.social, driven by Playwright Chrome against the built `dist/` (preview server). Two happy-path tests, serialized (one worker — they share the test accounts): alice publishes / bob subscribes-and-pins-the-channel; and an interrupted publish resuming from its IndexedDB checkpoint on reload. Plus a guarded maintenance task that drains accumulated test channels. |
