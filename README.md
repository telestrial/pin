...
- **Object packing.** Every Sia upload pays a full slab of erasure-coded redundancy regardless of content size — small text items are inefficient. `sdk.uploadPacked()` is the fix; would also amortize the per-attachment slab cost on photo-heavy posts.
