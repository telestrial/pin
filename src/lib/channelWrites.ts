...
export async function createAndPublishChannel(
  ...
  const created = await createChannel(sdk, args)
...
export async function saveChannelEdits(
  ...
  const current = await loadCurrentManifest(sdk, channel)
  const { manifest, reclaimURLs } = await editChannel(sdk, current, patch)
  await commitChannelManifest(
    sdk,
    channel.channelID,
    channel.channelKey,
...
export async function retractChannel(
  ...
  try {
    current = await loadCurrentManifest(sdk, channel)
  } catch {
